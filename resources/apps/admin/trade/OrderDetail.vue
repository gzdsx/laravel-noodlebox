<template>
    <main-layout>
        ...
        <section class="page-section">
                        ...
                        <div class="order-item">
                            ...
                            <div class="flex">
                                ...
                                <div class="item-metas">
                                    <dl v-for="(meta,index) in item.meta_data" :key="index">
                                        <dt>{{ meta.key }}</dt>
                                        <dd>{{ metaValues(meta) }}</dd>
                                    </dl>
                                </div>
                            </div>
                        </div>
            ...
            <template v-if="order.payment_status&&transaction">
                <div class="edit-title">
                    <strong>付款信息</strong>
                </div>
            ...
            <template v-if="order.shipping_status">
                <div class="edit-title">
                    <strong>物流信息</strong>
                </div>
                <table class="dsxui-formtable">
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            if (!order_id) return;
            ApiService.get(`/solds/${order_id}`).then(response => {
                this.order = response.data;
                const {shipping, transaction, order_fee} = response.data;
        ...
        fetchExpressList() {
            this.$get('/express/list').then(response => {
                this.expressList = response.data.items;
            });
        },
        onChange(val) {
            //console.log(val);
        ...
        metaValues(meta) {
            return meta.value.map(item => item.title).join(',');
        },
